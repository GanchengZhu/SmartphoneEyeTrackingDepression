...
public class ScalerMoveView extends View implements View.OnTouchListener {
    ...
    public ScalerMoveView(Context context) {
        this(context, null);
    }

    public ScalerMoveView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ScalerMoveView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
...
//                    setSelfPivot(lessX, lessY);
                    ...
                    if (scale > SCALE_MIN && scale < SCALE_MAX) {
                        setScale(scale);
                        SharedPreferencesUtils.setFloat(Constants.SCALE, scale);
    ...
    private void setSelfPivotY(float lessY) {
        float setPivotY = 0;
        setPivotY = getPivotY() + lessY;
        Log.i(getClass().getSimpleName(), "setPivotY:" + setPivotY
                + "  getWidth:" + getWidth() + "  getHeight:" + getHeight());
        if ( setPivotY < 0) {
            setPivotY = 0;
        } else if (setPivotY > 0) {
        ...
        setPivotY(setPivotY);
        SharedPreferencesUtils.setFloat(Constants.PIVOT_Y, setPivotY);
        SharedPreferencesUtils.setFloat(Constants.PIVOT_X, getPivotX());
    }
    /**
     * 计算两个点的距离
     *
